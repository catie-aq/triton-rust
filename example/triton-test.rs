...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    triton_inferer.unregister_system_shared_memory("image_input_data").await?;
    triton_inferer.unregister_system_shared_memory("output_data").await?;

    let size_of_image_input: u64 = 3 * 512 * 512 * 4;
    let size_of_classes_output: u64 = 4 * 6;
    let size_of_reg_output: u64 = 4;

    let mut system_mem_zone_input = triton_inferer.create_system_shared_memory("image_input_data", "/image_input_simple", size_of_image_input).await?;
    let mut system_mem_zone_output = triton_inferer.create_system_shared_memory("output_data", "/output_simple", size_of_classes_output + size_of_reg_output).await?;

    let response = triton_inferer.system_shared_memory_status("image_input_data").await?;
    println!("{:?}", response);
    let response = triton_inferer.system_shared_memory_status("output_data").await?;
    println!("{:?}", response);
    ...
    let response  = triton_inferer.infer("proditec-efficientnet", "1", "25", infer_inputs, Vec::<InferRequestedOutputTensor>::new(), data_inputs).await?;
    println!("{:?}", response);
    ...
    let response  = triton_inferer.infer("proditec-efficientnet", "1", "25", infer_inputs, infer_outputs, Vec::<Vec<u8>>::new()).await?;
    ...
    let response = triton_inferer.unregister_system_shared_memory("image_input_data").await?;
    println!("{:?}", response);
    let response = triton_inferer.unregister_system_shared_memory("output_data").await?;
    println!("{:?}", response);
